...
export class AddProductComponent extends SharedMessagesComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.screenTitleNavigationService.setScreenKey('AddProduct');
  ...
  onSaveButtonClick() {
    this.isProcessing = true;
    this.saveProductQuantity(this.returnProductsWithdrawalRequestDTO());
  }
  ...
  saveProductQuantity(requestDTO: Object) {
    let subscribtion = this.productsService.saveProductQuantity(requestDTO).subscribe(
      (response: any) => {
        this.isProcessing = false;
